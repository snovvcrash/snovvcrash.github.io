...
Раз нам недвусмысленно дали понять, что это ключ (для подключения по SSH, очевидно), соберем его в отдельный файл:
```
root@kali:~# cat hype_key | xxd -ps -r > rsa_key
```

Ключ зашифрован парольной фразой, поэтому просто так к серверу нам подключиться не дадут.
...
```
root@kali:~# chmod 600 rsa_key_decrypted
```
