...
Подробнее об этом способе можно узнать из [этого](https://www.youtube.com/watch?v=k6ri-LFWEj4 "VulnHub - Sokar - YouTube") туториала (0:15:36-0:39:10) прохождения машины с VulnHub'а. От себя добавлю, что в нашем случае представляется невозможным использование библиотеки `requests` для Python в силу особенностей используемого эксплойта: в `requests` не реализована адекватная обработка исключения `http.client.IncompleteRead`, которое в нашем случае имеет место быть при каждом втором запросе, поэтому приходится использовать встроенные средства языка. Подробнее об этой проблеме [здесь](https://github.com/mazen160/struts-pwn/issues/8 "Issue with requests partial read · Issue #8 · mazen160/struts-pwn").
...
Выдвину предположение, что это авторизационные данные для подключение по SSH (и не прогадаю :smiling_imp:)!
