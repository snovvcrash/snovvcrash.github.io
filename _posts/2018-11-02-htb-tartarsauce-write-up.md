...
Мы же злостно проэксплуатрируем эту опцию, подсунув tar'у вместо утилиты для архивирования обычный `/bin/sh` с волшебными редиректами для входных и выходных данных, что дарует нам пользовательский шелл:
```text
...
Из вывода `uname -a` мы уже видели, что TartarSauce имеет 32-х битную архитекутру, поэтому компилируем следующим образом:
```text
...
Право на запись в `/var/www/html/webservices` имеет толко владелец — root.
