...
2\. Дописать на коленке обработку POST-запрос для Python-сервера, это тоже нетрудно:
```python
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
*sunny* не сможет надеть маску root'а, это под силу только *sammy*. Именно поэтому бесполезно менять `UID:GID` существующих пользователей, ровно как и добавлять новых — ведь их в списоке ролей не будет и подавно. Подробнее почитать про RBAC можно [здесь](https://blogs.oracle.com/solaris/understading-rbac-v2 "Understading RBAC / Oracle Solaris Blog").
...
Наш случай второй. Так как это bash-скрипт, то выполнять (выполнять == **прочитать содержимое** и запустить) его будет, очевидно, bash-интерпретатор, который в свою очередь запущен от имени *sunny*, и у которого поэтому не достаточно прав для прочтения файла со скриптом. Тайна раскрыта!
