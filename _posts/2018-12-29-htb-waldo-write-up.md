...
**Waldo** — несложная Linux-коробка, внутри которой затерялся злосчастный Вальдо. Неотъемлемой частью процедуры поиска последнего станут такие развлечения, как эксплуатация уязвимого к атаке типа "*Path Traversal + LFI*" веб-приложения, врыв в docker-контейнер по SSH, обнаружение альтернативного SSH-ключа и подключение к реальной виртуальной машине с помощью оного, побег из *restricted-shell*'а (*rbash*) и, на сладкое, чтение файлов с правами root через абьюзинг приложения с установленным мандатом *CAP_DAC_READ_SEARCH* из арсенала одного из механизмов управления доступом в Линукс — *Linux Capabilities*. Итак, где же Вальдо? **Сложность: 5/10**{:style="color:grey;"}
...
## Burp Suite
Пропустив трафик через локальную проксю Burp'а и осмотревшись на сайте, я составил следующий маппинг "действие → скрипт.php", где *действие* — функция, предоставленная приложением (менеджером списков), *скрипт.php* — PHP-скрипт, отвечающий за выполнение соответствующий функции:
...
Значит, картина складывается примерно такая: мы подключаемся к 22-у порту SSH к ВМ Waldo (10.10.10.87), после чего посредством магии маршрутизации нас бросает в docker-контейнер (172.17.0.1) на порт 8888. Следовательно, было бы неплохо выбраться отсюда в "реальный мир" виртуалки Waldo.
