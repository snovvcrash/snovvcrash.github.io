---
layout: post
title: "В королевстве PWN. Обходим DEP и брутфорсим ASLR на HTB{ October }"
...
[![xakep-badge.png]({{ "/img/xakep-badge.png" | relative_url }})](https://xakep.ru/2019/10/08/hackthebox-dep-aslr/ "В королевстве PWN. Обходим DEP и брутфорсим ASLR на виртуалке с Hack The Box - «Хакер»")

**October** — относительно несложная виртуальная машина, однако на ее примере удобнее всего разобрать, что в сущности из себя представляет атака ret2libc, применяемая для обхода запрета выполнения данных (DEP/NX-Bit) в стеке, и подбор необходимого адреса той самой стандартной разделяемой библиотеки libc для нивелирования рандомизации размещения адресного пространства (ASLR). Ко всему прочему, на общий уровень сложности повлиял челлендж с захватом админки CMS, где случайно оставили дефолтную авторизацию, поэтому быстро пробежим вступление и более подробно остановимся на этапе privilege escalation.
...
Машину для нового прохождения я подбирал, основываясь не только на ее названии (октябрь же!), но и на специфике предлагаемых ею испытаний — низкоуровневая эксплуатация, что отлично подходит для текущий серии статей. В этот раз нас ждет исследование October CMS, получение веб-шелла, разведка на хосте с целью выявления уязвимого бинарника и комбинация атаки «ret2libc + брутфорс ASLR» для получения сессии суперпользователя.

**4.7/10**
{: style="color: orange; text-align: right;"}

[![banner.png]({{ "/img/htb/boxes/october/banner.png" | relative_url }})](https://www.hackthebox.eu/home/machines/profile/15 "Hack The Box :: October")
{: .center-image}
...
![browser-index.png]({{ "/img/htb/boxes/october/browser-index.png" | relative_url }})
{: .center-image}
...
![browser-backend.png]({{ "/img/htb/boxes/october/browser-backend.png" | relative_url }})
{: .center-image}

Оказавшись в админке, недолго думая, я попробовал стандартные `admin:admin`.

![browser-admin-upload.png]({{ "/img/htb/boxes/october/browser-admin-upload.png" | relative_url }})
{: .center-image}
...
![browser-admin-backdoor.png]({{ "/img/htb/boxes/october/browser-admin-backdoor.png" | relative_url }})
{: .center-image}

Теперь я могу инициировать выполнение команд от имени `www-data`, обратившись к загруженному файлу с аргументом `cmd`.

![browser-admin-rce.png]({{ "/img/htb/boxes/october/browser-admin-rce.png" | relative_url }})
{: .center-image}
...
![burp.png]({{ "/img/htb/boxes/october/burp.png" | relative_url }})
{: .center-image}

После отправки запроса можно ловить отклик на netcat.

![nc-listen.png]({{ "/img/htb/boxes/october/nc-listen.png" | relative_url }})
{: .center-image}
...
![nc-upgrade-pty.png]({{ "/img/htb/boxes/october/nc-upgrade-pty.png" | relative_url }})
{: .center-image}

Таким образом я оказался внутри машины и начал осматриваться вокруг.

![user-txt.png]({{ "/img/htb/boxes/october/user-txt.png" | relative_url }})
{: .center-image}
...
![linenum-sh.png]({{ "/img/htb/boxes/october/linenum-sh.png" | relative_url }})
{: .center-image}
...
![ovrflw-glance.png]({{ "/img/htb/boxes/october/ovrflw-glance.png" | relative_url }})
{: .center-image}
...
![october-check-aslr.png]({{ "/img/htb/boxes/october/october-check-aslr.png" | relative_url }})
{: .center-image}
...
![ovrflw-checksec.png]({{ "/img/htb/boxes/october/ovrflw-checksec.png" | relative_url }})
{: .center-image}
...
![ovrflw-readelf.png]({{ "/img/htb/boxes/october/ovrflw-readelf.png" | relative_url }})
{: .center-image}
...
![ovrflw-assembly.png]({{ "/img/htb/boxes/october/ovrflw-assembly.png" | relative_url }})
{: .center-image}
...
![localhost-check-aslr.png]({{ "/img/htb/boxes/october/localhost-check-aslr.png" | relative_url }})
{: .center-image}
...
![stack-layout-leave-ret.png]({{ "/img/htb/boxes/october/stack-layout-leave-ret.png" | relative_url }})
{: .center-image}
...
![stack-layout-ret2libc.png]({{ "/img/htb/boxes/october/stack-layout-ret2libc.png" | relative_url }})
{: .center-image}
...
![gdb-ovrflw-eip-offset.png]({{ "/img/htb/boxes/october/gdb-ovrflw-eip-offset.png" | relative_url }})
{: .center-image}
...
![gdb-ovrflw-maps.png]({{ "/img/htb/boxes/october/gdb-ovrflw-maps.png" | relative_url }})
{: .center-image}
...
![procfs-ovrflw-maps.png]({{ "/img/htb/boxes/october/procfs-ovrflw-maps.png" | relative_url }})
{: .center-image}
...
![exploit_no_aslr-py.png]({{ "/img/htb/boxes/october/exploit_no_aslr-py.png" | relative_url }})
{: .center-image}
...
![aslr-changing-bytes-localhost.png]({{ "/img/htb/boxes/october/aslr-changing-bytes-localhost.png" | relative_url }})
{: .center-image}
...
![aslr-changing-bytes-pic.png]({{ "/img/htb/boxes/october/aslr-changing-bytes-pic.png" | relative_url }})
{: .center-image}
...
![exploit-py.png]({{ "/img/htb/boxes/october/exploit-py.png" | relative_url }})
{: .center-image}
...
![october-get-addresses.png]({{ "/img/htb/boxes/october/october-get-addresses.png" | relative_url }})
{: .center-image}

И проведу эксплуатацию прямо из Bash.

![october-ovrflw-pwn.png]({{ "/img/htb/boxes/october/october-ovrflw-pwn.png" | relative_url }})
{: .center-image}
