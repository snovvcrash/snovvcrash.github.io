...
1. An account with SPN set (we'll abuse `MachineAccountQuota` to create one) that will impersonate (delegate to) another user (`HTB\lee`) to access target computer service (`http/WEB.htb.local`) via PtT by abusing protocol transition mechanism (S4U2Self & S4U2Proxy).
