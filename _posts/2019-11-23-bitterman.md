...
> 3. **➤**{:.green} [ROP-цепочки и атака Return-to-PLT в CTF Bitterman](/2019/11/23/bitterman.html)
...
Первый кейс покажет различия в эксплуатации Stack Smashing относительно этой же атаки в 32-битной ОС (о которой мы говорили [в первой части](/2019/10/20/classic-stack-overflow.html)) в случае, когда у нарушителя есть возможность размещения и выполнения шелл-кода в адресном пространстве стека — то есть с отключенными защитами DEP/NX и ASLR.

Второй кейс поможет разобраться в проведении атаки ret2libc для x86-64 (ее 32-битный аналог был рассмотрен [во второй части](/2019/11/08/htb-october.html)). Здесь мы обсудим, какие регистры использует 64-битный ассемблер Linux при формировании стековых кадров, а также посмотрим, что в сущности из себя представляет концепция Return-oriented programming (ROP). Механизм DEP/NX активен, ASLR — нет.
...
Достаточно долгое время единственной защитой от ROP-атак являлся механизм ASLR. Однако в частых случаях он не является значимым препятствием для нарушителя: адрес libc можно перебрать [методом грубой силы](/2019/11/08/htb-october.html#обход-aslr--метод-грубой-силы) (если речь идет о 32-битных системах), или воспользоваться несовершенством написанного кода, абьюзя библиотечные функции, чтобы спровоцировать утечку памяти (как будет показано на примере заключительного тренировочного кейса).
...
* *(строки 14-23)* Далее формируется полезная нагрузка, шаблон которой мы обсудили выше. Смещение ROP-гаджета уже было найдено, а адреса остальных частей пейлоада можно добыть с помощью отладчика, как мы делали это [здесь](/2019/11/08/htb-october.html#1-адрес-загрузки-libc). Функция `p64`, как ты мог догадаться, выполняет то же самое, что и `little_endian` в сплоите из первого кейса.
...
В [четвертой](/2019/12/20/htb-smasher.html) (и заключительной) части серии мы разберем тачку Smasher с Hack The Box, изюминкой прохождения которой стал низкоуровневый сплоитинг веб-сервера. Stay tuned!
